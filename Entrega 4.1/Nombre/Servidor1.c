	...
	for (;;){
		...
		while (terminar ==0)
		{
			...
			if (codigo ==0) //peticion de desconexion
				...
				sprintf (respuesta,"%s tiene %d letras",nombre, strlen (nombre));

			else if (codigo ==2) //el nombre bonito
				if((nombre[0]=='A') || (nombre[0]=='E') || (nombre[0]=='I') || (nombre[0]=='O') || (nombre[0]=='U'))
					strcpy (respuesta,"Ese nombre es bonito");
				else
					strcpy (respuesta,"Ese nombre no es bonito");
		    
			else if (codigo ==3) //nombre palindromo
			    {
					int a,x,y;
					a=strlen(nombre);
					a=a-1;
